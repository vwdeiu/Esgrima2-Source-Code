package Esgrima2_fla
{
   import flash.accessibility.*;
   import flash.display.*;
   import flash.errors.*;
   import flash.events.*;
   import flash.external.*;
   import flash.filters.*;
   import flash.geom.*;
   import flash.media.*;
   import flash.net.*;
   import flash.net.drm.*;
   import flash.system.*;
   import flash.text.*;
   import flash.text.ime.*;
   import flash.ui.*;
   import flash.utils.*;
   
   public dynamic class warrior_lighing_strike_1304 extends MovieClip
   {
       
      
      public var body:MovieClip;
      
      public var fx1:POINT;
      
      public var e3:MovieClip;
      
      public var hand1:MovieClip;
      
      public var e4:MovieClip;
      
      public var foot1:MovieClip;
      
      public var hand2:MovieClip;
      
      public var foot2:MovieClip;
      
      public var hat:MovieClip;
      
      public var arm1:MovieClip;
      
      public var wep:MovieClip;
      
      public var head:MovieClip;
      
      public var arm2:MovieClip;
      
      public var effect2:MovieClip;
      
      public var effect3:MovieClip;
      
      public var fist1:MovieClip;
      
      public var fx:POINT;
      
      public var leg1:MovieClip;
      
      public var fist2:MovieClip;
      
      public var leg2:MovieClip;
      
      public var leg3:MovieClip;
      
      public var bag:MovieClip;
      
      public var leg4:MovieClip;
      
      public var synchronize:Vector.<String>;
      
      public function warrior_lighing_strike_1304()
      {
         var _loc1_:Boolean = false;
         var _loc2_:Boolean = true;
         if(!_loc1_)
         {
            super();
            if(!_loc1_)
            {
               addr23:
               addFrameScript(0,this.frame1);
            }
            return;
         }
         §§goto(addr23);
      }
      
      function frame1() : *
      {
         var _loc1_:Boolean = true;
         var _loc2_:Boolean = false;
         if(_loc1_)
         {
            this.synchronize = Vector.<String>(["effect1","effect2","effect3","effect4","effect5","e1","e2","e3","e4"]);
         }
      }
   }
}
